...
export class FormValidator {
    ...
    _setEventListener() {
        this._toggleButtonState (this._submitButtonElement, this._formElement.checkValidity(), config);
        // 4 Перебираем список инпутов конкретной формы и вешаем на каждый инпут обработчик событий input
        [...this._inputList].forEach((inputElement) => {
            inputElement.addEventListener('input', () => {
                this._toggleButtonState (this._submitButtonElement, this._formElement.checkValidity(), config);
                this._checkInputValidity(inputElement, this._formElement, config);
            })
        })
